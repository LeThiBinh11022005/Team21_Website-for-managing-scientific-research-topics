...
<main class="flex-1 overflow-auto p-6">
    <div class="max-w-7xl mx-auto">
        ...
        <form method="GET" class="mb-4 flex items-center gap-3">
            <label for="role" class="font-medium">Lọc theo phân quyền:</label>
            <select name="role" id="role" onchange="this.form.submit()" class="border rounded px-3 py-2">
        ...
        <div class="mb-4 flex gap-3">
            <a href="account/create_account.php" class="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                Thêm tài khoản
            </a>
            <button id="deleteSelected" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 hidden"
                    onclick="deleteSelectedAccounts()">Xóa nhiều</button>
        </div>

        <!-- Bảng -->
        <form id="accountsForm" method="POST" action="../../handle/account_process.php?action=delete_multiple">
                ...
                <tbody class="divide-y divide-gray-200">
                    <?php if (!empty($accounts)): ?>
                        ...
                        <tr class="hover:bg-gray-100">
                            <td class="py-2 px-3">
                                <input type="checkbox" class="selectItem" name="ids[]" value="<?= $acc['id'] ?>">
                            </td>
                            <td class="py-2 px-4"><?= htmlspecialchars($acc['id']) ?></td>
                            <td class="py-2 px-4"><?= htmlspecialchars($acc['account_name']) ?></td>
                            <td class="py-2 px-4"><?= htmlspecialchars($acc['email']) ?></td>
                            <td class="py-2 px-4">
                                <?php 
                                    if ($acc['role'] == 'student') echo 'Sinh viên';
                                    elseif ($acc['role'] == 'lecturer') echo 'Giảng viên';
                                    else echo 'Quản trị';
                                ?>
                            </td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="py-4 text-gray-500">Chưa có tài khoản nào</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
